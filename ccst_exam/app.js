...
function sectionLabel(section) {
  ...
  if (section === "single") return "單選/配對題";
...
function renderQuestion() {
  ...
  questionTitle.textContent = `第 ${question.number} 題（第 ${question.page} 頁）`;
...
async function init() {
  ...
  metaText.textContent = `總題數 ${meta.total} 題｜是非 ${meta.sections["true-false"]}｜單選/配對 ${meta.sections.single}｜複選 ${meta.sections.multiple}｜圖像 ${meta.sections.image}`;

  jumpSelect.innerHTML = "";
  state.data.questions.forEach((q, idx) => {
    const opt = document.createElement("option");
    opt.value = String(idx);
    opt.textContent = `${idx + 1}. ${sectionLabel(q.section)} 第${q.number}題`;
