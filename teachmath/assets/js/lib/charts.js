...
export function renderBarChart(values, highlightLabel) {
    const width = 560;
    const height = 240;
...
export function renderDotPlot(values, mean) {
    const width = 560;
    const height = 160;
    const padding = 36;
    ...
    const scale = (value) => padding + ((value - min) / spread) * (width - padding * 2);
    const points = values
        .map((value, index) => {
        const x = scale(value);
...
export function renderNormalCurve(mean, standardDeviation, lower, upper, marker) {
    const width = 560;
    const height = 260;
    ...
    const xScale = (value) => padding.left + ((value - minX) / (maxX - minX)) * chartWidth;
    const yScale = (density) => padding.top + chartHeight - (density / maxDensity) * chartHeight;
