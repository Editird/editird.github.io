...
const examContainer = document.querySelector("[data-exam-drills]");
if (lessonRoot) {
    renderChapterLessonList(lessonRoot, chapterLessons.probability);
}
if (practice) {
...
renderLatex();
function renderChapterLessonList(container, lessons) {
    if (lessons.length === 1) {
    ...
    lessons.forEach((lesson) => {
        const target = container.querySelector(`[data-chapter-theater="${lesson.id}"]`);
        if (target) {
