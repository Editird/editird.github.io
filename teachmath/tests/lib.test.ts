...
import { examGuideLessons, probabilityLesson } from "../src/content/guidedLessons";
import { examDrills } from "../src/content/practice";
import { questionTypeItems, questionTypeOptions } from "../src/content/questionTypes";
import { binomialProbability, combination, factorial, fixedSequenceProbability } from "../src/lib/binomial";
...
describe("guided lesson operation steps", () => {
  it("keeps solve theater steps atomic and free of concept-card fields", () => {
    for (const lesson of examGuideLessons) {
      expect(lesson.steps.length).toBeGreaterThan(0);
      expect("summary" in lesson).toBe(false);

      for (const step of lesson.steps) {
        expect(step.instruction.length).toBeGreaterThan(0);
        expect(step.instruction.split(/[。!?？]/).filter(Boolean)).toHaveLength(1);
        expect("microCues" in step).toBe(false);
        expect("microInstruction" in step).toBe(false);
        expect("detail" in step).toBe(false);
        expect("tokens" in step).toBe(false);

        if (step.stage) {
          expect("bubble" in step.stage).toBe(false);
          expect("writeLabel" in step.stage).toBe(false);
          expect("focusLabel" in step.stage).toBe(false);
          expect("cue" in step.stage).toBe(false);
        }
      }
    }
  });

  it("splits the expected value example into single-operation screens", () => {
    expect(probabilityLesson.steps.slice(0, 10).map((step) => step.instruction)).toEqual([
      "列出期望值公式。",
      "取第一組數值。",
      "把 0 乘以 0.2。",
      "取第二組數值。",
    ...
    expect(probabilityLesson.steps.length).toBeGreaterThanOrEqual(20);
  });
});
